...
chainId == 31337
  ? describe("Dynamic SVG NFTs", () => {
      ...
      beforeEach(async () => {
        ...
        deployer = accounts[0];
        await deployments.fixture(["dynamicNft", "mocks"]);
        dynamicNft = await ethers.getContract("DynamicSvgNft");
        MockV3Aggregator = await ethers.getContract("MockV3Aggregator");
      });

      describe("Constructor", () => {
        it("It should initialize the value in Constructor Properly!", async () => {
      ...
      describe("Mint NFT", () => {
        it("emits an event and creates the NFT", async function () {
          ...
          expect(await dynamicNft.mintNft(highValue)).to.emit(
            dynamicNft,
            "createdNFT"
          );
        });
        it("It increases the tokenCounter by 1", async () => {
          const highValue = ethers.utils.parseEther("1"); // 1 dollar per ether
